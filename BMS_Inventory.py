...
def add_product(categoria_object, marca_object, modelo_object, serie_object, pais_object, inv_num_object, descripcion_object, precio_base_object, precio_ask_object, tables_list, root, quantity_object, user_id):
	...
	if len(marca) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Brand is Missing")
	elif len(modelo) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Model is Missing")
	elif len(serie) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Serial # is Missing")
	elif len(pais) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Country is Missing")
	elif len(inv_num) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Inventory # is Missing")
	elif len(descripcion) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Description is Missing")	

	## PRICES DONT MATTER IF THEY ARE EMPTY RIGHT NOW ##
		# elif len(precio_base) == 0:
		# 	add_prod = False
		# 	label_warning = tk.Label(root, text="WARNING: Base Price is Missing")
		# elif len(precio_ask) == 0:
		# 	add_prod = False
		# 	label_warning = tk.Label(root, text="WARNING: Asking Price is Missing")

	elif (precio_base.replace('.', '', 1).isdigit() == False) and len(precio_base) > 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Base Price must be a number")
	elif precio_ask.replace('.', '', 1).isdigit() == False and len(precio_ask) > 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Asking Price must be a number")
	elif len(quantity) == 0:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Quantity is Missing")
	elif quantity == '0':
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Quantity cannot be 0")
	elif (quantity.replace('.', '', 1).isdigit() == False):
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Quantity must be a number")
	elif (float(quantity).is_integer() == False):
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Quantity must be a whole number")
	else:
		label_warning = tk.Label(root, bg='#D9D9D9')
	...
	if bool(check_serial_existance) == True and (serie != "N/A"):
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Serial # Already in Inv")
		label_warning.place(relx=0.3, rely=0.35, relwidth=0.3, relheight=0.06)
		if add_prod == False:
			return
	...
	if bool(checking_inv_num_existance) == True:
		add_prod = False
		label_warning = tk.Label(root, font=add_prod_font, bg='#D9D9D9', fg='red', text="WARNING: Inv # Already in Inv")
		label_warning.place(relx=0.3, rely=0.35, relwidth=0.3, relheight=0.06)
		if add_prod == False:
			return
	...
	if int(quantity) > 1:
		...
		button_yes = tk.Button(root_sameordiff, bg='#B6FE90', text='YES', font='Calibri 12', command=lambda: yes(root_sameordiff, quantity, specs_no_serial_inv, serie, inv_num, user_id, serial_state, modelo))
		...
		button_no = tk.Button(root_sameordiff, bg='#FE9090', text='NO', font='Calibri 12', command=lambda: no(root_sameordiff, quantity, specs_no_desc_price_serial_inv, serie, inv_num, descripcion, precio_base, precio_ask, user_id, serial_state, modelo))
	...
	# Quantity = 1
		...
		conn_main.commit()
		#tk.messagebox.showinfo("Import", "Item Added Successfully", parent=root)


		# entrada_product_excel(category_id, marca, modelo, serie, pais, inv_num, descripcion, precio_base, precio_ask, user_id)


def yes(root_sameordiff, quantity, specs_no_serial_inv, serie, inv_num, user_id, serial_state, model):
	...
	button_enter = tk.Button(root_yes, font='Calibri 16', bg='light blue', text='ENTER', command=lambda: next_serial(entry_serial, entry_inv_num, serial_list, inv_num_list, label_outof, quantity, root_yes, specs_no_serial_inv, user_id, serial_state, model, label_warning))
	button_enter.place(relx=0.825, rely=0.4, relwidth=0.15, relheight=0.35)


def no(root_sameordiff, quantity, specs_no_desc_price_serial_inv, serie, inv_num, descripcion, precio_base, precio_ask, user_id, serial_state, model):
	global current_serial
	...
	button_enter = tk.Button(root_no, font='Calibri 16', bg='light blue', text='ENTER', command=lambda: next_serial_desc_price(entry_serial, entry_inv_num, entry_desc, entry_precio_base, entry_precio_ask, serial_inv_desc_price_list, label_outof, quantity, root_no, specs_no_desc_price_serial_inv, user_id, serial_state, model, label_warning))
...
def next_serial(serial_num_object, inv_num_object, serial_list, inv_num_list, label_outof, quantity, root_yes, specs_no_serial_inv, user_id, serial_state, model, label_warning):
	global current_serial
	...
	if current_serial > int(quantity):
		root_yes.destroy()
		add_product_same_desc_price(quantity, serial_list, inv_num_list, specs_no_serial_inv, user_id)
	else:	
		inv_num_object.delete(0, 'end')
...
def add_product_same_desc_price(quantity, serial_list, inv_num_list, specs_no_serial_inv, user_id): # This function is for adding products that have the same price and desc
	...
	for i in range(int(quantity)):
		...
		cur_main.execute(insert_query, insert_data)
		conn_main.commit()
		tk.messagebox.showinfo("Import", "Items Added Successfully", parent=root)

def next_serial_desc_price(serial_num_object, inv_num_object, desc_object, price_base_object, price_ask_object, serial_inv_desc_price_list, label_outof, quantity, root_no, specs_no_desc_price_serial_inv, user_id, serial_state, model, label_warning):
	global current_serial
	...
	if current_serial > int(quantity):
		root_no.destroy()
		add_product_diff_desc_price(quantity, serial_inv_desc_price_list, specs_no_desc_price_serial_inv, user_id)
	else:
		serial_num_object.delete(0, 'end')
...
def add_product_diff_desc_price(quantity, serial_inv_desc_price_list, specs_no_desc_price_serial_inv, user_id): # This function is for adding products with diff desc and prices
